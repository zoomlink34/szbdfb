...
function checkStatus() {
    ...
    db.ref('notifications/' + id).once('value', s => {
        const d = s.val();
        if(!d) alert("Order is being reviewed by CEO.");
        else alert(`--- HISTORY RECORD ---\nID: ${id}\nSTATUS: ${d.status}\nCEO MESSAGE: ${d.message}`);
    });
}
